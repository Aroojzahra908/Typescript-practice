...
var positiveNumbers = numbers.filter((num1:number)=> {
    return num1 >= 0;
});
...
var multipliedNumbers = numbers.map((num2:number)=> {
    return num2 * 2;
});
console.log("Original array:", numbers);
...
var longFruits = fruits.filter((fruit:any)=> {
...
var evenSquaredNumbers = nums.filter((numb:number)=> {
    return numb % 2 === 0              // Filter even numbers
})
    var arr2:number[]= nums.map((evenNumber:number) =>{
...
var ar4:number[]=numbers.map((oddNumber)=> {
